...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height, showtime=False):
    ...
    if showtime:
        print("get_lidar_in_image_fov cost %.2f ms" %
              ((time.perf_counter() - time1) * 1000))
...
def dataset_viz():
    ...
    split = 'training'
    save2ddir = os.path.join(ROOT_DIR, 'dataset/KITTI', split, 'vis2d')
    save3ddir = os.path.join(ROOT_DIR, 'dataset/KITTI', split, 'vis3d')
...
def dataset_viz_pred(pred_label_dir):
    ...
    save2ddir = os.path.join(ROOT_DIR, 'dataset/KITTI', split, 'vis2d')
    save3ddir = os.path.join(ROOT_DIR, 'dataset/KITTI', split, 'vis3d')
    save2ddir_pred = os.path.join(
        ROOT_DIR, 'dataset/KITTI', split, 'vis2d_pred')
    save3ddir_pred = os.path.join(
        ROOT_DIR, 'dataset/KITTI', split, 'vis3d_pred')
    if os.path.isdir(save2ddir) == True:
        print('previous save2ddir found. deleting...')
        shutil.rmtree(save2ddir)
    os.makedirs(save2ddir)
